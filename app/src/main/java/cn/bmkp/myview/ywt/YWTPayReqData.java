...
public class YWTPayReqData {
     ...
     * (选填) 过期时间跨度,
     必须为大于零的整数，单位为分钟。
     该参数指定当前支付请求必须在指定时间跨度内完成（从系统收到支付请求开始计时），
    ...
    /**
     * (选填) 成功页返回商户地址,支付成功页面上“返回商户”按钮跳转地址。为空则不显示“返回商户”按钮。
     * 原生APP可传入一个特定地址（例如:Http://CMBNPRM），并拦截处理自行决定跳转交互。
     */
    public String returnUrl = "";
    /**
     * (选填) 商户取得的客户IP，如果有多个IP用逗号”,”分隔。
     */
    ...
    /**
     * 客户协议号。支持数字、字母（大小字母）、-_两个特殊字符（-_注意是英文半角的）。未签约（首次支付）客户，填写新协议号，用于协议开通；
     * 已签约（再次支付）客户，填写该客户已有的协议号。商户必须对协议号进行管理，确保客户与协议号一一对应。
     */
    public String agrNo = "";
    /**
     * (选填) 协议开通请求流水号，开通协议时必填。
     */
    ...
    /**
     * (选填) 经度，商户app获取的手机定位数据，如30.949505
     */
    public String lon = "";
    /**
     * (选填) 纬度，商户app获取的手机定位数据，如50.949506
     */
    public String lat = "";
    /**
     * (选填) 风险等级:用户在商户系统内风险等级标识
     */
    ...
    /**
     * (选填) json格式写入的扩展信息，并使用extendInfoEncrypType指定的算法加密
     使用详见扩展信息
